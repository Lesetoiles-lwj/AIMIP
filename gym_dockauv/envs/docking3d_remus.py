...
class BaseDocking3d_remus(gym.Env):
    ...
    def reward_step(self, action: np.ndarray) -> float:
        ...
        # velocity_reward = self.reward_factors["w_velocity"] * np.linalg.norm(self.auv.ned_velocity[0:2])  # todo dai: 这里想用横向上的速度作为奖励，

        v_auv = self.auv.state[6:9]
        v_cu = self.current(self.auv.state[3:6],position=self.auv.state[0:3])[0:3]
        cos_v = np.dot(v_auv, v_cu) / (np.linalg.norm(v_auv) * np.linalg.norm(v_cu))
        velocity_reward = self.reward_factors["w_velocity"] * np.linalg.norm(v_cu) * cos_v  #todo wenjie: 用洋流和速度夹角作为奖励，
